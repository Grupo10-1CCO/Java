...
public class AutenticacaoApiCrud {
      ...
      List <AutenticacaoApi> checagem = cursor.query(String.format("SELECT * FROM AutenticacaoApi WHERE emailUsuario LIKE '%s' AND senha LIKE '%s' AND serialMaquina LIKE '%s'",email, senha, serial), new BeanPropertyRowMapper(AutenticacaoApi.class));
